...
function [z, stat] = step_split(obj,t,z,dt)
	...
	function z = step_advect_diffuse(t,z)
		if (1 == obj.ndim)
			z_ = reshape(z,[obj.nx(1),sum(obj.aux.isactive)]);
		...
		case {'step_advect_diffuse_q'}
			if (1 == obj.ndim)
				idx_ = 0;
				q = obj.opt.time_integration.q;
				for idx=1:obj.nvar
					if (obj.aux.isactive(idx))
						idx_ = idx_+1;
						z_(:,idx_) = (obj.aux.Ix - (dt*q)*obj.aux.Axi{idx_}) ...
								\ (z_(:,idx_) + (dt*(1-q))*(obj.aux.Axi{idx_}*z_(:,idx_)));
					end % if isactive
				end % for idx
			else % of ndim == 1
		...
		case {'step_advect_diffuse_implicit_q_fft'}
			...
			for idx=1:obj.nvar
				if (1 == obj.ndim)
					% only integrate active variables
					% (for decativating surface flow during dry periods)
					if (obj.aux.isactive(idx))
					idx_ = idx_+1;
					z_(:,idx_) = step_advect_diffuse_implicit_q_fft( ...
					 dt, ...
					 dx, ...
					   [obj.pmu.vx{idx_}], ...
					    [obj.pmu.ex{idx_}], ...
		...
		z = reshape(z_,sum(obj.aux.isactive)*prod(obj.nx),1);
